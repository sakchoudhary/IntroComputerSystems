...
void* event_handler(void* arg) {
    ...
    string requests[w];
    RequestChannel *rc[w];
	RequestChannel *eventChannel = wkr.channel;


    //cout << "inside event_handler"<< endl;
    for(int i = 0; i < w; i++){
        string s2 = eventChannel->send_request("newthread");
        rc[i] =  new RequestChannel(s2, RequestChannel::CLIENT_SIDE);
        requests[i] = wkr.b_buffer->retrieve_front(); 
        rc[i]->cwrite(requests[i]); 
        send_counter++; 

    }
    //cout << "done making new channels in event_handler"<< endl;

    int max_num = 0; 
    while(recv_counter < 3*n) {
        //cout << "recv_counter"<< recv_counter << endl;

        fd_set fds;
	    //FD_ZERO(&fds);
	    for(int i = 0; i < w; ++i)
	    {
	    	FD_SET(rc[i]->read_fd(), &fds);
            if(max_num < rc[i]->read_fd()){
                max_num = rc[i]->read_fd();
            }
	    }
        int k = select(max_num+1,&fds,NULL,NULL,NULL); 
        for(int i = 0; i < w; i++){
            if(FD_ISSET(rc[i]->read_fd(),&fds)){
                string message = rc[i]->cread();
                ...
                if(send_counter < 3*n){
                    send_counter++;
                    string rq = wkr.b_buffer->retrieve_front(); 
                    requests[i] = rq; 
                    rc[i] ->cwrite(rq);
                }
                recv_counter++; 
...
int main(int argc, char * argv[]) {
    ...
    */
        ...
        pthread_create(&worker_thread, NULL, event_handler, &w1);
    ...
    */
    ...
    /*
        JOIN WORKER THREADS
    */
        cout << "waiting for worker thread to join... "; 
